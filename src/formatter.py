...
def competition_icon(name: str) -> str:
    return "🏆" if "Champions" in name else "⚽"


def matches_for_day(feed: dict, day=None) -> list[dict]:
    if day is None:
        day = datetime.now(OSLO).date()
...
def format_daily_message(matches: list[dict], demo: bool = False) -> str:
    if not matches:
        return ""
    ...
    if demo:
        title = "🧪 <b>DEMO</b> – " + title.replace("<b>", "").replace("</b>", "")
    ...
    chunks += [
        "",
        "⏰ Tidene vises i norsk tid.",
        "ℹ️ TV-oppsett kan endres av rettighetshaver.",
    ]
    return "\n".join(chunks)
